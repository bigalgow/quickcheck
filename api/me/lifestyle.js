// api/me/lifestyle.js
// API endpoint for lifestyle profile CRUD operations
...
export default async function handler(req, res) {
  try {
    ...
    const currentProfile = current.lifestyleProfile ?? null;

    if (req.method === 'GET') {
      // Return the lifestyle profile if it exists
    ...
    if (req.method === 'DELETE') {
      // Clear lifestyle profile
      const next = { ...current, lifestyleProfile: null };
      await updateUserCustomData(userId, next, mgmtToken);
      return res.status(204).end();
    }
